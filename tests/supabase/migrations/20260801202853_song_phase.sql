-- Song phase (2C): resolve_song for post-song continue/decline, and the
-- no-follow-up rule — a declined dance-floor match closes the conversation
-- for both send paths (regular and event).

-- ============================================================
-- RESOLVE_SONG — continue (match stays active) or decline (closed)
-- ============================================================
create or replace function public.resolve_song(p_match_id uuid, p_continue boolean)
returns void
...
begin
  ...
  if not exists (
    ...
    where m.id = p_match_id
      and (m.user_id_a = v_user or m.user_id_b = v_user)
      and m.source = 'dance_floor'
  ) then
    raise exception 'not_your_song';
  end if;

  update public.matches
  set status = case when p_continue then 'active' else 'declined' end
  where id = p_match_id;
end;
$$;

grant execute on function public.resolve_song(uuid, boolean) to authenticated;

-- ============================================================
-- NO-FOLLOW-UP RULE — declined matches freeze the conversation
-- ============================================================
...
begin
  ...
  -- No-follow-up: a declined match closes the conversation.
  if exists (
    select 1 from public.matches m
