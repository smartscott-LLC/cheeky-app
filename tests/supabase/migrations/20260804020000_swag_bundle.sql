-- Bundle codes (2026-08-04): one code, many benefits. The Lions Den mints a
-- single SWAG code carrying {"tokens": N, "gifts": ["slug"], "membership": "tier"}
-- for the "event glitch kit" style gestures — entry back, tokens, and a
-- teddy in one print. Owner/system only; redemption applies each benefit in
-- one atomic pass (no partial grants).

alter table public.swag_codes
  drop constraint if exists swag_codes_benefit_type_check,
  add constraint swag_codes_benefit_type_check
    check (benefit_type in ('membership', 'tokens', 'gift', 'bundle'));

alter table public.benefit_grants
  drop constraint if exists benefit_grants_benefit_type_check,
  add constraint benefit_grants_benefit_type_check
    check (benefit_type in ('membership', 'tokens', 'gift', 'bundle'));

-- ============================================================
-- GENERATE SWAG CODE — bundle-aware
-- ============================================================
...
-- ============================================================
-- REDEEM SWAG CODE — bundle-aware. One atomic pass: tokens to the ledger,
-- gifts to inventory, membership to the entitlement grant.
-- ============================================================
  ...
  if v_row.benefit_type = 'membership' then
    ...
    if jsonb_typeof(v_payload->'gifts') = 'array' then
      for v_gift in select jsonb_array_elements_text(v_payload->'gifts') loop
        insert into public.gift_inventory (user_id, catalog_id, status)
        values (v_user, (select id from public.gift_catalog where slug = v_gift), 'available');
      end loop;
    end if;
  end if;
