...
void test(
  ...
  async (t) => {
    ...
    await t.test(
      'fresh member: tier + zero usage, not checked in',
      async () => {
        ...
        assert.equal(
          own.status,
          200,
          'member can read their own profile (RLS intact)'
        );
    ...
    await t.test(
      'check-in flips the flag; messages move the usage counts',
      async () => {
        ...
        assert.equal(
          beforeRead[0].messages_sent_today,
          0,
          'A sent nothing yet'
        );
        ...
        assert.equal(
          afterSend[0].new_people_today,
          1,
          'B counts as a new person'
        );
      }
    );
  }
