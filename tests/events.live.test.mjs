...
void test(
  ...
  async (t) => {
    ...
    await t.test(
      'the hourly wheel: scheduled kinds on the quarter, live',
      async () => {
    ...
    for (const [kind, floor, cost] of [
      ...
      await t.test(
        `${kind}: mutual pick -> match, holds -> debits`,
        async () => {
          ...
          const joinA = await rpcAs(
            a.id,
            `select public.join_event('${ev}')`,
            []
          );
          ...
          assert.equal(
            await balance(admin, a.id),
            cost + 5 - cost,
            'A debited the cost'
          );
          assert.equal(
            await balance(admin, b.id),
            cost + 5 - cost,
            'B debited the cost'
          );
    ...
    await t.test(
      ...
      async () => {
        ...
        await rpcAs(
          members[0].id,
          `select public.submit_rooftop_pick('${ev}', 0, '${members[1].id}')`,
          []
        );
        await rpcAs(
          members[1].id,
          `select public.submit_rooftop_pick('${ev}', 0, '${members[0].id}')`,
          []
        );
        await rpcAs(
          members[2].id,
          `select public.submit_rooftop_pick('${ev}', 0, '${members[3].id}')`,
          []
        );
        await rpcAs(
          members[3].id,
          `select public.submit_rooftop_pick('${ev}', 0, '${members[2].id}')`,
          []
        );
        await rewind();
        ...
        assert.equal(
          evState.status,
          'closed',
          'pool closed after the final pair'
        );
        ...
        assert.equal(
          matches.length,
          3,
          'three couples matched (2 rounds + the final 1v1)'
        );
    ...
    await t.test(
      'rooftop pool: an odd pool refunds the lone leftover',
      async () => {
        const ev = await mkEvent('rooftop', 'diamond', 40, 60);
        const members = [];
        for (let i = 0; i < 7; i++) {
        ...
        for (const [a, b] of [
          ...
          await rpcAs(
            members[a].id,
            `select public.submit_rooftop_pick('${ev}', 0, '${members[b].id}')`,
            []
          );
          await rpcAs(
            members[b].id,
            `select public.submit_rooftop_pick('${ev}', 0, '${members[a].id}')`,
            []
          );
        }
        await rewind();
        ...
        assert.equal(
          leftover.status,
          'released',
          'the lone leftover is refunded'
        );
        assert.equal(
          await balance(admin, members[6].id),
          50,
          'leftover untouched'
        );
    ...
    await t.test(
      ...
      async () => {
        ...
        await rpcAs(
          u[0].id,
          `select public.select_speed_rank('${ev}', 1::smallint, '${u[1].id}')`,
          []
        );
        await rpcAs(
          u[1].id,
          `select public.select_speed_rank('${ev}', 1::smallint, '${u[0].id}')`,
          []
        );
        await rpcAs(
          u[2].id,
          `select public.select_speed_rank('${ev}', 1::smallint, '${u[3].id}')`,
          []
        );
        await rpcAs(
          u[3].id,
          `select public.select_speed_rank('${ev}', 1::smallint, '${u[2].id}')`,
          []
        );
        await rpcAs(
          u[4].id,
          `select public.select_speed_rank('${ev}', 2::smallint, '${u[5].id}')`,
          []
        );
        await rpcAs(
          u[5].id,
          `select public.select_speed_rank('${ev}', 2::smallint, '${u[4].id}')`,
          []
        );
        ...
        assert.equal(
          certs.length,
          6,
          'all six matched members hold a certificate'
        );
    ...
    await t.test(
      'date night: matched pair, mutual taps lock the round',
      async () => {
        ...
        await rpcAs(
          a.id,
          `select public.tap_date_night('${gid}', 0, 2::smallint)`,
          []
        );
    ...
    await t.test(
      ...
      async () => {
        ...
        await admin
          ...
          .insert({ user_id: host.id, ...grant });
        for (const s of suitors)
          await admin
            .from('entitlement_grants')
            .insert({ user_id: s.id, ...grant });
        ...
        for (const s of suitors) {
          const j = await rpcAs(
            s.id,
            `select public.join_blind_date('${ev}')`,
            []
          );
    ...
    await t.test(
      ...
      async () => {
        ...
        try {
          ...
          const reserved =
            await one`select count(*)::int as n from public.event_entries where event_id = ${ev} and status = 'reserved'`;
          assert.equal(reserved[0].n, N, 'all reserved before finalize');

          await one`select public.finalize_events()`;
          const [{ status }] =
            await one`select status from public.events where id = ${ev}`;
          assert.equal(status, 'closed', 'event closed after the cycle');
          const released =
            await one`select count(*)::int as n from public.event_entries where event_id = ${ev} and status = 'released'`;
          assert.equal(
            released[0].n,
            N,
            `all ${N} holds released (no-match refund)`
          );
          const debited =
            await one`select count(*)::int as n from public.token_ledger where user_id in ${one(members.map((m) => m.id))} and delta < 0`;
