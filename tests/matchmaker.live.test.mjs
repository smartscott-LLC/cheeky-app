...
void test(
  ...
  async (t) => {
    ...
    t.after(async () => {
      ...
      if (unlockIds.length)
        await chunkIn('matchmaker_unlocks', 'id', unlockIds);
    ...
    await t.test(
      'draft phase: filtered candidates, 2-pick cap, no real likes',
      async () => {
        const crew = [];
        ...
        assert.ok(
          Array.isArray(cands) && cands.length >= 2,
          'candidates present'
        );
        assert.ok(
          cands.every((c) => c.id !== a.id),
          'never the caller'
        );
        assert.ok(
          cands.every((c) => c.photo_path),
          'every candidate has a photo'
        );
        assert.ok(
          cands.every((c) => c.picked === false),
          'nothing picked yet'
        );
    ...
    await t.test(
      'start_board: 16 cards, 8 pairs, the drafts are the stakes',
      async () => {
    ...
    await t.test(
      'flip: reveal, match, strike, and the 2-match win',
      async () => {
        const { data: cards } = await admin
          .from('matchmaker_cards')
    ...
    await t.test(
      'send_unlock: one first impression per matched pair, its own allowance',
      async () => {
        ...
        const { data: unlockId, error } = await rpc(
          a.token,
    ...
    await t.test(
      'accept: chat opens, both can talk, recipient earns the sender-floor gift',
      async () => {
        const { data: matchedCards } = await admin
          .from('matchmaker_cards')
          .select('target_id')
    ...
    await t.test(
      ...
      async () => {
        ...
        const { data: brows, error: bErr } = await rpc(
          b.token,
          'matchmaker_start_board',
          {}
        );
        ...
        const { data: unlockId, error: uErr } = await rpc(
          b.token,
          'matchmaker_send_unlock',
          {
        ...
        assert.ok(
          u2.gift_inventory_id,
          'consolation gift linked to the unlock'
        );
        ...
        assert.equal(
          (bgifts ?? []).length,
          1,
          'sender got the consolation gift'
        );
    ...
    await t.test(
      '3 strikes lose — a board with one match ends quiet',
      async () => {
        const { data: cards } = await admin
          .from('matchmaker_cards')
    ...
    await t.test(
      'plays dial: silver burns two boards, then the third is refused',
      async () => {
        ...
        const { data: rows, error: e2 } = await rpc(
          b.token,
          'matchmaker_start_board',
          {}
        );
        ...
        await admin
          .from('matchmaker_cards')
          .delete()
          .eq('board_id', rows[0].board_id);
        await admin
          .from('matchmaker_boards')
          .delete()
          .eq('id', rows[0].board_id);
        ...
        assert.equal(
          ts[0].matchmaker_plays_left,
          0,
          'taskbar reports 0 plays left'
        );
      }
    );

    await t.test(
      'exclusivity: buy_gift refuses the matchmaker-only items',
      async () => {
        const { error } = await rpc(a.token, 'buy_gift', {
          p_slug: 'matchmaker_silver'
        });
        assert.ok(
