...
// (e.g. AGNES/agnes-2.5-flash). Auth via VERCEL_OIDC_TOKEN
...
export function streamAgent(opts: {
  ...
  // AGNES-v3 is reachable on the Vercel free tier; agnes-2.5-flash
  // resolves to a paid provider. Override via AI_MODEL anytime.
  const model = process.env.AI_MODEL ?? 'AGNES/AGNES-v3';
  return streamText({
    model,
    system: opts.system,
