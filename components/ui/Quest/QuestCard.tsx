...
export default function QuestCard({ data }: { data: CardData }) {
  ...
  // Membership icon
  const membershipIcon = data.membershipLevel === 'diamond' ? '💎'
  ...
  const backDesign = isFemale
    ? <div className="absolute inset-0 flex items-center justify-center gap-2"><span className="text-6xl">❤️</span><span className="text-6xl">❤️</span></div>
    : <div className="absolute inset-0 flex items-center justify-center"><span className="text-7xl opacity-80">🛡️</span></div>;
    ...
    >
      <div className={`relative w-full h-full transition-transform duration-500 transform-style-3d ${flipped ? 'rotate-y-180' : ''}`}>
        ...
        <div className="absolute inset-0 rounded-2xl overflow-hidden rotate-y-180 backface-hidden"
          ...
          <div className="absolute top-3 left-3 z-20 text-2xl" style={{ filter: 'drop-shadow(0 0 8px rgba(255,215,0,0.8))' }}>
            {membershipIcon}
          </div>
          ...
          <div className="relative px-4 pt-2 pb-12 min-h-24">
            ...
            {data.bio && (
              <div className="relative font-body text-sm text-gray-300 leading-relaxed"
                   style={{
          ...
          <div className="absolute bottom-4 right-4 z-10 w-14 h-14 rounded-full overflow-hidden border-2 border-gold"
            ...
            <div className="flex justify-between text-xs font-damion text-cyan">
              ...
              {(data.hometown || data.state) && (
                <span className="text-right">
                  {data.hometown}{data.state ? `, ${data.state}` : ''}
                </span>
              )}
            </div>
          </div>
