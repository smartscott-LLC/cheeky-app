    ...
    return (
      <div className="bg-black">
        <div className="mx-auto max-w-md px-6 py-20">
          <p className="text-center text-5xl font-body text-club">🦁</p>
          <h1 className="font-hero text-gold mt-4 text-center text-3xl">
            The Lions Den
          </h1>
          <p className="mt-2 text-center text-sm font-body text-club">
          ...
          <button
            onClick={() => unlock(key)}
            disabled={busy}
            className="mt-3 w-full rounded-lg border border-zinc-700 px-6 py-2.5 font-bold text-cyan transition hover:border-zinc-500 disabled:opacity-40"
          >
            Unlock
          </button>

          <div className="my-6 flex items-center gap-3 text-[10px] uppercase tracking-[0.3em] text-cyan">
            <div className="h-px flex-1 bg-zinc-800" />
            not the owner?
            <div className="h-px flex-1 bg-zinc-800" />
          </div>

          <p className="text-center text-sm font-body text-club">
          ...
          {msg && (
            <p
              className={`font-body text-club mt-4 text-center text-sm ${
                msg.ok ? 'text-emerald-400' : 'font-body text-club'
              }`}
            >
              {msg.text}
            </p>
  ...
  return (
    <div className="bg-black">
      <div className="mx-auto max-w-4xl px-6 py-16">
        ...
        {msg && (
          <p
            className={`font-body text-club mt-3 text-sm ${msg.ok ? 'text-emerald-400' : 'font-body text-club'}`}
          >
            {msg.text}
          </p>
        )}
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            ...
            >
              <p className="text-2xl font-extrabold font-body text-club">
                {m.icon} {m.value.toLocaleString()}
              </p>
              <p className="mt-1 text-[11px] uppercase tracking-wider font-body text-club">
                {m.label}
              </p>
            </div>
          ))}
            ...
            <div className="mt-3 space-y-3">
              {flags.map((f) => (
                ...
                >
                  ...
                  <p className="mt-1 text-sm italic font-body text-club">
                    “{f.reason}”
                  </p>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    <button
                      ...
                      disabled={busy}
                      className="rounded-lg border border-zinc-600 px-4 py-1.5 text-sm font-bold text-cyan hover:border-zinc-400 disabled:opacity-40"
                    >
                      Dismiss
                    </button>
                  </div>
            ...
            <div className="mt-3 space-y-3">
              {reports.map((r) => (
                ...
                >
                  ...
                  <p className="mt-0.5 font-mono text-[10px] font-body text-club">
              ...
              <span className="mt-1.5 flex flex-wrap gap-1.5">
                {['AGNES-chat', 'AGNES-reasoner'].map((m) => (
                  <button
                    ...
                    className="rounded border border-zinc-700 px-2 py-0.5 text-[11px] text-cyan hover:border-gold hover:text-gold"
                  >
                    {m}
                  </button>
                ))}
                ...
                >
                  <span
                    className={`w-20 font-bold capitalize ${
                      closed ? 'font-body text-club' : 'text-white'
                    }`}
                  >
                    {f.name}
                  </span>
                  {closed ? (
                    <>
                      <span className="text-xs font-bold uppercase font-body text-club">
                        🔧 under construction
                        {c.until
                      ...
                      <button
                        ...
                        disabled={busy}
                        className="ml-auto rounded border border-zinc-600 px-3 py-1 text-xs font-bold text-cyan hover:border-zinc-400 disabled:opacity-40"
                      >
                        Reopen
                      </button>
                    </>
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {events.map((e) => (
              <div
                ...
                className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-3"
              >
                <p className="text-sm font-bold capitalize font-body text-club">
        ...
        <div className="mt-8">
          ...
          <div className="mt-3 space-y-1.5">
            {ledger.length === 0 && (
              <p className="text-sm font-body text-club">No movement yet.</p>
            )}
            {ledger.map((r) => (
              <div
                key={r.id}
              ...
              >
                <span
                  className={`font-bold ${
                    r.delta > 0 ? 'text-emerald-400' : 'font-body text-club'
                  }`}
                >
            ...
            <div className="mt-3 space-y-2">
              {staleCodes.map((c) => (
                ...
                >
                  ...
                  <span className="text-xs font-body text-club">
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div>
            ...
            <div className="mt-3 space-y-2">
              {codes.length === 0 && (
                <p className="text-sm font-body text-club">None yet.</p>
              )}
              {codes.map((c) => (
                <div
                  key={c.id}
                ...
                >
                  <div>
                    <button
                      onClick={() => copy(c.code)}
                      className="font-mono font-body text-club hover:underline"
                      title="Copy"
                    >
                      {c.code}
                    </button>
        ...
        {mintOpen && (
          ...
          >
            <div className="absolute right-0 top-0 h-full w-full max-w-md overflow-y-auto border-l border-zinc-800 bg-zinc-950 p-6">
              ...
              <div className="mt-5 grid grid-cols-2 gap-2">
                {PRESETS.map((p) => (
                  <button
                    ...
                    className="rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-xs font-bold text-cyan transition hover:border-gold hover:text-gold"
                  >
                    {p.label}
                  </button>
                ))}
              ...
              <div className="mt-5 space-y-2">
                {mintRows.map((row) => (
                  <div key={row.id} className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="text-cyan hover:font-body text-club"
                    >
                      ✕
                    </button>
                  </div>
                ...
                <button
                  onClick={addMintRow}
                  className="text-xs font-bold font-body text-club hover:font-body text-club-cotton"
                >
                  + Add a benefit
                </button>
              </div>
